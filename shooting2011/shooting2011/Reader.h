...
private:
	Reader();
	static EnemyData* get_enemy_data(string pictureId){

		int start_time,hp;
		double x,y;
		
		start_time=atoi(format.c_str());
		if(ss>>x>>y>>hp>>format){
			...
			GraphicPattern *gp=new GraphicPattern(pictureId);
			return(new EnemyData(start_time,hp,x,y,mp,fp,gp));
		}
		return NULL;
	}
